...
function matches(employee) {
  const f = state.filters;
  const haystack = [employee.name, employee.purpose, employee.note, employee.longNote, ...employee.tagIds.map((id) => label(state.tags, id))].join(" ").toLowerCase();
